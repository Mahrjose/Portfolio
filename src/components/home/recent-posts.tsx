...
import { Calendar, ArrowRight, ExternalLink, Clock, ChevronLeft, ChevronRight } from "lucide-react"
...
export function RecentPosts() {
  ...
  const scrollLeft = () => {
    if (scrollRef.current) {
      const cardWidth = scrollRef.current.querySelector(".blog-card")?.clientWidth || 0
      const gap = 16 // 1rem gap
      scrollRef.current.scrollBy({ left: -(cardWidth * 3 + gap * 2), behavior: "smooth" })
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-12 animate-fade-in-up">
          <div className="mb-6 sm:mb-0">
            ...
            <p className="text-muted-foreground text-lg">Recent thoughts on AI and machine learning</p>
          </div>
            ...
            {blog.map((post, index) => (
                ...
                <div className="relative h-48 flex-shrink-0 overflow-hidden rounded-t-md">
                  <div className="absolute inset-0 overflow-hidden">
                    <Image
                      src={post.image || "https://images.unsplash.com/photo-1677442136019-21780ecad995?w=600&h=300&fit=crop"}
                      alt={post.title}
                      fill
                ...
                <CardContent className="pt-0 flex-1 flex flex-col">
                  <div className="flex flex-wrap gap-2 mb-4">
                    {post.tags.slice(0, 3).map((tag) => (
                      <Badge key={tag} variant="outline" className="text-xs badge-hover">
                        {tag}
                      </Badge>
                    ))}
                  </div>
